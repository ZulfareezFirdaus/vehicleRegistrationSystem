...
.show {display: block;}
 ...
 $query =  "Select (Select count(*) from studentreport where gender='L' AND lesen = 'ada' ) AS LAL, (Select count(*) from studentreport where gender='L' AND lesen = 'tiada' ) AS LTL, (Select count(*) from studentreport where gender='P' AND lesen = 'ada' ) AS PAL, (Select count(*) from studentreport where gender='P' AND lesen = 'tiada' ) AS PTL";  
    ...
    <script type="text/javascript">
      ...
      function drawChart()  {  
                var data = google.visualization.arrayToDataTable([  
                          ['LESEN', 'LELAKI','PEREMPUAN'],  
                          <?php  
while($row = mysqli_fetch_array($result))  
                          {
		    echo "['ADA', ".$row["LAL"]." ,".$row["PAL"]."],";  
                                echo "['TIADA', ".$row["LTL"]." ,".$row["PTL"]."],";   
        ...
        var options = {
          chart: {
            ...
            subtitle: 'JANTINA KESELURUHAN BAGI LESEN ',
          }
        };
