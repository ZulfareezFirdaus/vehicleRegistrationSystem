...
.show {display: block;}
 ...
 $query =  "Select (Select count(*) from studentreport where gender='L' AND form = 3 ) AS L3, (Select count(*) from studentreport where gender='L' AND form = 4 ) AS L4, (Select count(*) from studentreport where gender='L' AND form = 5 ) AS L5, (Select count(*) from studentreport where gender='P' AND form = 3 ) AS P3 ,(Select count(*) from studentreport where gender='P' AND form = 4 ) AS P4, (Select count(*) from studentreport where gender='P' AND form = 5 ) AS P5";  
    ...
    <script type="text/javascript">
      ...
      function drawChart()  {  
                var data = google.visualization.arrayToDataTable([  
                          ['JANTINA', 'TINGKATAN 3','TINGKATAN 4','TINGKATAN 5'],  
                          <?php  
while($row = mysqli_fetch_array($result))  
                          {
		    echo "['LELAKI', ".$row["L3"]." ,".$row["L4"].",".$row["L4"]."],";  
                                echo "['PEREMPUAN', ".$row["P3"]." ,".$row["P4"].", ".$row["P5"]."],";   
        ...
        var options = {
          chart: {
            ...
            subtitle: 'TINGKATAN KESELURUHAN BERDASARKAN JANTINA  ',
          }
        };
